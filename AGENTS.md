...
- Bind every XHS profile to the current PGY creator identity. Accept a saved profile URL only when its route identity matches the PGY creator-detail identity; do not resolve from page-wide links or a brittle click on the visible XHS ID. A nested notes/feed loader must not override stable visible profile identity.
